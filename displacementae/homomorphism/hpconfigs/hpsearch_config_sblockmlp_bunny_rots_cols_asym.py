...
grid = {
    ...
    ### Dataset options ###
    'dataset': ['obj3d'],
    # 'data_root': ['/fast/hkeurti/datasets/obj3d/dataset0/transitions.hdf5'],
    'data_root': ['/fast/hkeurti/datasets/obj3d/dataset9/transitions.hdf5'],
    'n_steps': [2],
    'rotate_actions':[0],
    # 'resample':[True],
    # 'resample_every':[2],
    ...
    'save_figures':[True],
    'rollouts':[True],
    'rollouts_path':['/fast/hkeurti/datasets/obj3d/dataset9/rollouts.hdf5'],
    # 'rollouts_path':['/fast/hkeurti/datasets/obj3d/dataset0/rollouts.hdf5'],
    'rollouts_batch_size':[10],
    'plot_rollouts':[True],
    'plot_n_rollouts':[5],
    ...
    ### network options ###
    'conv_channels': ['"32,32,32,32"','"64,64,64,64"'],
    'kernel_sizes': ['"6,4,4,4"'],
    'strides': ['"2,2,1,1"'],
    'lin_channels': ['"256,256"','"64,64"'],

    # 'decoder_conv_channels': ['"64,64,64,64"','"64,64"','"128,128"'],
    'decoder_conv_channels': ['"64,64,64,64"'],
    # 'decoder_kernel_sizes': ['"4,4"','"4,6"','"4,4,4,6"'],
    'decoder_kernel_sizes': ['"4,4,4,6"'],
    'decoder_strides': ['"1,1,2,2"'],
    # 'decoder_lin_channels': ['"128,128,128"','"256,256"'],
    'decoder_lin_channels': ['"256,256"', '"64,64"'],

    'variational': [False],
    'beta': [0],
    # 'normalize_post_action':[False,True],
    'latent_loss_weight':[10,100,200,400,800,1200],
    'latent_loss':[True],
    ...
    ### Evaluation options ###
    'val_epoch' : [10],
    'wandb_project_name' : ['icmlcr_sblock_bunny_rots_cols_9.0'],
    'log_wandb':[True],
    'checkpoint':[True],
    ...
    'grp_loss_on':[True],
    'grp_loss_weight':[0.05, 0.1, 0.2, 0.5, 1, 2],
    'regularize_algebra':[True, False],    
}
...
conditions = [
    ...
    # ({'strides': ['"2,2,1,1"','"1,1,1,1"']},{'kernel_sizes': ['"6,4,4,4"'],'conv_channels': ['"64,64,64,64"','"32,32,32,32"','"128,128,128,128"','"128,256,512,512"','"256,512,512,1024"']}),
    # # ({'strides': ['"4,2,1,1"']},{'kernel_sizes': ['"8,5,3,3"'],'conv_channels': ['"64,64,64,64"','"32,32,32,32"','"128,128,128,128"','"128,256,512,512"','"256,512,512,1024"','"128,256,256,512"']}),
    ...
    # ({'reconstruct_first':[True]},{'reconstruct_first_only':[False]}),
    # ({'reconstruct_first':[False]},{'reconstruct_first_only':[True]}),
    # ({'decoder_conv_channels': ['""']},{'decoder_strides': ['""','""'],'decoder_kernel_sizes': ['""']}),
    # ({'decoder_strides': ['"2,2"','"1,1"']},{'decoder_kernel_sizes': ['"6,4"','"4,4"'],'decoder_conv_channels': ['"32,32"','"128,128"','"64,64"']}),
    # ({'decoder_strides': ['"2,2,1"','"1,1,1"']},{'decoder_kernel_sizes': ['"6,4,4"'],'decoder_conv_channels': ['"32,32,32"','"64,64,64"']}),
    # ({'decoder_strides': ['"1,1,2,2"','"1,1,1,1"']},{'decoder_kernel_sizes': ['"6,4,4,4"'],'decoder_conv_channels': ['"32,32,32,32"','"16,16,16,16"','"64,64,64,64"']}),
...
_SCRIPT_NAME = '-W ignore train_hae.py soft_block_mlp'
...
# >>> _ARGPARSE_HANDLE = f
import __init__
import displacementae.homomorphism.train_args as targs

# _ARGPARSE_HANDLE = lambda argv : targs.parse_cmd_arguments( \
#     representation=Representation.BLOCK_MLP, argv=argv)

_ARGPARSE_HANDLE = lambda argv : targs.parse_cmd_arguments(argv=argv)
